...
/*   Updated: 2025/01/23 21:11:47 by mohmazou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../Includes/cub3d_bonus.h"

void	put_pixel_rec(mlx_image_t *img, int x, int y, int color)
{
	if (x < 0 || x >= TIL_SIZE * 10)
		return ;
	if (y < 0 || y >= TIL_SIZE * 10)
		return ;
...
void	draw_rect(t_game *game, int x, int y, int color)
{
	...
	while (i < TIL_SIZE)
	{
		...
		while (j < TIL_SIZE)
		{
			put_pixel_rec(game->img, x + j, y + i, color);
			j++;
		}
		i++;
	}
